...
def knapsack_bound_6d(sigma_pairs, caps_u, caps_w, sigma1, caps_v, tau_u, tau_w, tau_v):
...
def optimize_tau_6d(sigma_pairs, caps_u, caps_w, sigma1, caps_v):
    P = len(sigma_pairs)
    K1 = len(sigma1)
    x0 = np.ones(2 * P + K1)
    ...
    def objective(x):
        ...
        tau_w = x[P : 2 * P]
        tau_v = x[2 * P :]
        return knapsack_bound_6d(
            sigma_pairs, caps_u, caps_w, sigma1, caps_v, tau_u, tau_w, tau_v
        )
...
def tensor_knapsack_bound_6d(t):
    ...
    tau_u_opt, tau_w_opt, tau_v_opt, bound_min = optimize_tau_6d(
        sigma_pairs, caps_u, caps_w, sigma1, caps_v
    )
